...
namespace Real::assets {
    ...
    void ModelManager::LoadAll(const String &rootDir) {
        namespace std_fs = std::filesystem;
        const auto& mm = Services::GetModelManager();
        ...
        for (const auto& entry : std_fs::directory_iterator(rootDir)) {
            if (entry.is_directory() && !mm.IsModelExist(entry.path().filename().string())) {
    ...
    Ref<Model> ModelManager::Load(const String &filePath, const String& name, ImageFormatState state)
    {
        ...
        Services::GetModelManager().RegisterModel(m_CurrentModel);
        Services::GetAssetImporter().SaveModelToAssetDB(m_CurrentModel);

        return m_CurrentModel;
    }

    void ModelManager::RegisterModel(const Ref<Model> &model) {
    ...
    Ref<Model> ModelManager::GetModel(const String &name) const {
    ...
    bool ModelManager::IsModelExist(const String &name) const {
    ...
    void ModelManager::ProcessMesh(const aiMesh *mesh, const aiScene *scene, const aiMatrix4x4& transform) {
        ...
        if (mesh->mMaterialIndex >= 0) {
            ...
            materialUUID = real_material ? real_material->id : materialUUID;
        }
    ...
    Ref<Material> ModelManager::ProcessMaterial(const aiMaterial *mat, int materialIndex) {
        ...
        aiString aiMatName;
        mat->Get(AI_MATKEY_NAME, aiMatName);
        const auto baseName = aiMatName.length > 0
            ? aiMatName.C_Str()
        ...
        auto AssignTexture = [&](const Ref<platform::opengl::OpenGLTexture>& tex, TextureType type) {
            ...
            switch (type) {
                case TextureType::AMBIENT_OCCLUSION:
                    ormPack[material->name][0] = tex; return;
                case TextureType::ROUGHNESS:
                    ormPack[material->name][1] = tex; return;
                case TextureType::METALLIC:
                    ormPack[material->name][2] = tex; return;
                default: break;
            }
        ...
        auto LoadTexture = [&] (aiTextureType aiType) {
            ...
            auto checkCache = [&](const String& key) -> bool {
                ...
                AssignTexture(tm.GetTexture(cachedUUID), realType);
                return true;
            };
            ...
            if (realType == TextureType::ALBEDO_ROUGHNESS) {
                ...
                if (raw.channelCount > 3) {
                    ...
                    albTex->SetDebugName(material->name + "_ALB");
                    albTex->CreateFromData(albData, TextureType::ALBEDO);
                    albTex->SetFileInfo(fs::FileInfoFromPath(destPath + material->name + "_ALB" + ext));
                    ...
                    rghTex->SetDebugName(material->name + "_RGH");
                    rghTex->CreateFromData(rghData, TextureType::ROUGHNESS);
                    ormPack[material->name][1] = rghTex;
                } else {
                    const auto tex = CreateTexture(resolvedPath,
                        destPath + material->name + "_ALB" + ext,
                        TextureType::ALBEDO);
                    AddTextureToMaterial(tex, material);
            ...
            if (const auto it = suffix.find(realType); it != suffix.end()) {
                const auto outPath = destPath + material->name + String(it->second) + ext;
        ...
        for (const auto& [matName, pack] : ormPack) {
            ...
            material->orm = tm.Register(orm);
        }

        return material;
    }

    void ModelManager::AddTextureToMaterial(
        ...
        const Ref<Material>& material)
    {
        ...
        if (!tex) return;
        switch (tex->GetType()) {
            case TextureType::ALBEDO:   material->albedo   = tm.FindByUUID(tex->GetUUID()); break;
            case TextureType::NORMAL:   material->normal   = tm.FindByUUID(tex->GetUUID()); break;
            case TextureType::HEIGHT:   material->height   = tm.FindByUUID(tex->GetUUID()); break;
            case TextureType::EMISSIVE: material->emissive = tm.FindByUUID(tex->GetUUID()); break;
            default: break;
        }
    }

    std::filesystem::path ModelManager::ChooseBest(const Vector<std::filesystem::path> &paths) {
        // Prefer by order
