...
namespace Real::UI {
    ...
    void InspectorPanel::OnSceneAttached(Scene *scene) {
        m_Scene = scene;
    }
    ...
    void InspectorPanel::DrawComponents() {
        ...
        if (entity->HasComponent<TransformComponent>()) {
            DrawComponent(&entity->GetComponentUnchecked<TransformComponent>());
        }
        if (entity->HasComponent<MeshRendererComponent>()) {
            DrawComponent(&entity->GetComponent<MeshRendererComponent>());
        }
        if (entity->HasComponent<ColliderComponent>()) {
            DrawComponent(*entity, &entity->GetComponent<ColliderComponent>());
        }
        if (entity->HasComponent<RigidbodyComponent>()) {
            DrawComponent(*entity, &entity->GetComponent<RigidbodyComponent>());
        }
        if (entity->HasComponent<LightComponent>()) {
            DrawComponent(&entity->GetComponent<LightComponent>(), &entity->GetComponent<TransformComponent>());
        }
    ...
    void InspectorPanel::DrawComponent(Entity& entity, ColliderComponent *comp) const {
        ...
        m_Scene->GetEvents().OnColliderChanged.Emit(
            entity.GetHandle(),
    ...
    void InspectorPanel::DrawComponent(CameraComponent *comp) {
        ...
        if (ImGui::CollapsingHeader("Editor Camera State")) {
            const auto& view = m_Scene->GetAllEntitiesWith<IDComponent, TagComponent, CameraComponent>();
